  ...
  if (streamFlag) {
    ...
    if ((len - STM_HEADER) % channels == 0) {
      ...
      if (loopable || inOffset) {
        if (color1) {
          if (!grayscale) setRGB(0, color1, STM_HEADER + idOffset);
          else setW(0,color1, STM_HEADER + idOffset);
        }
        if (color2) { 
          if (!grayscale) setRGB(1, color2, STM_HEADER + idOffset + colorOffset);
          else setW(1,color2, STM_HEADER + idOffset + colorOffset);
        }
